...
const RecipeIcon = ({ source }: { source: FavoriteRecipe['source'] }) => {
    if (source === 'country') return <Globe className="h-5 w-5 text-primary"/>;
    if (source === 'pantry') return <ChefHat className="h-5 w-5 text-primary"/>;
    if (source === 'custom') return <NotebookPen className="h-5 w-5 text-primary"/>;
...
export function FavoriteRecipes({ open, onOpenChange }: FavoriteRecipesProps) {
  ...
  const [selectedRecipe, setSelectedRecipe] = useState<Partial<FavoriteRecipe> | null>(null);
  ...
  const handleEditClick = (recipe: FavoriteRecipe) => {
    setSelectedRecipe(recipe);
    setIsFormOpen(true);
  };

  const handleSaveRecipe = async (data: { name: string; ingredients: string[]; instructions: string[] }) => {
    const isUpdating = !!selectedRecipe?.id;
    const result = isUpdating
      ? await updateCustomRecipe(selectedRecipe.id!, data)
      : await addCustomRecipe(data);

    if (result.success) {
      toast({
        title: `Recette ${isUpdating ? "mise à jour" : "ajoutée"} !`,
        description: `"${data.name}" a bien été ${isUpdating ? "mise à jour" : "sauvegardée"}.`,
      });
      fetchRecipes();
    } else {
      toast({
        variant: "destructive",
        title: `Erreur de ${isUpdating ? "mise à jour" : "sauvegarde"}`,
        description: result.message,
      });
    }
  };
          ...
          <div className="flex-1 overflow-y-auto -mx-6 px-6 py-4">
            {isLoading ? (
                  ...
                  <AccordionItem value={recipe.id} key={recipe.id}>
                    <AccordionTrigger>
                      <div className="flex items-center gap-3 text-left">
                          <RecipeIcon source={recipe.source} />
                      ...
                      <div className="space-y-4 pl-2">
                          ...
                          <div className="flex items-center gap-2 mt-4">
                            ...
                            {recipe.source === 'custom' && (
                                <Button variant="secondary" size="sm" className="w-full" onClick={() => handleEditClick(recipe)}>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground p-4">
                  ...
                  <p className="mt-1 text-sm">Cliquez sur "Ajouter" pour créer votre première recette.</p>
              </div>
            )}
          </div>
          <SheetFooter>
